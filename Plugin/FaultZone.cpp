...
std::string GMSH_FaultZonePlugin::getHelp() const
{
  return "Plugin(FaultZone) convert all the embedded lines of an existing "
         ...
         "Set a value different to zero can be helpful to check the "
         "connectivity. "
         "\n\n"
         "`Prefix' is the prefix of the name of physicals containing the new "
         "embedded. "
         "All physicals containing embedded lines are replaced by physicals "
         "containing "
...
PView *GMSH_FaultZonePlugin::execute(PView *view)
{
  ...
  if(itl == embeddedEdges.end()) {
    Msg::Error("No line to treat in this surface");
    Msg::Error(
      "The plugin FaultZone may have been already run for this surface. "
  ...
  if(eps) faultZoneMesher.ModifyJointNodePosition(eps / 2);
  ...
  for(std::vector<GEdge *>::const_iterator itl = embeddedEdges.begin();
    ...
    for(std::size_t i = 0; i < gEdge->getNumMeshElements(); i++) {
      ...
      for(its = _jointElements.begin(); its != _jointElements.end(); its++) {
        ...
        // the MElements are considered the same if the quadratic node is the
        // same
      ...
      if(its != _jointElements.end()) {
        Msg::Warning("Element edge %d appears in a second GEntity",
                     mElem->getNum());
        continue;
      }

      // the MElements are considered connected if the quadratic node is
      // connected
      std::set<MVertex *>::iterator its2 =
        allConnectedQuadraticVertices.find(mElem->getVertex(2));
      if(its2 == allConnectedQuadraticVertices.end()) {
        Msg::Warning(
          "Element edge %d seams to be not connected, it will be ignored",
          mElem->getNum());
        continue;
      }
    ...
    std::map<MVertex *, GEdge *>::iterator itm =
      _fissureByHeavNode.find(mVertexBegin);
...
void GMSH_FaultZoneMesher::DuplicateNodes()
{
  ...
  for(std::map<MVertex *, std::vector<GEdge *> >::iterator itm =
        _fissuresByJunctionNode.begin();
      itm != _fissuresByJunctionNode.end();) {
...
void GMSH_FaultZoneMesher::ComputeHeavisideFunction()
{
  for(std::map<MVertex *, std::vector<GEdge *> >::iterator itm =
    ...
    for(std::vector<GEdge *>::iterator itl = fissures.begin();
        itl != fissures.end(); itl++) {
...
//================================================================================
/*!
 * \brief Return the value of the heaviside function associated to mVert
 * evaluated on point sPoint.
 */
//================================================================================
std::vector<int> GMSH_FaultZoneMesher::HeavisideFunc(MVertex *mVert,
                                                     SPoint3 &sPoint)
{
 ...
 * a corresponding face element is created, based on the nodes of the edge
 * element. additionnal nodes are picked in the _nodeJointByHeavOrJunctionNode,
 * _nodeByHeavNode and _nodesByJunctionNode maps. In case of
 * _nodesByJunctionNode, the _heavFuncByJunctionNode map is used to found the
 * corresponding node.
 *
                                               ...
                                               std::string prefix)
{
  ...
  for(std::vector<GEdge *>::const_iterator itl = embeddedEdges.begin();
    ...
    GFace *jointFace =
      new discreteFace(gModel, gModel->getMaxElementaryNumber(2) + 1);
    ...
    for(std::size_t i = 0; i < gEdge->getNumMeshElements(); i++) {
      ...
      MQuadrangle8 *mQuad8 =
        new MQuadrangle8(mVerts[0], mVerts[1], mVerts[2], mVerts[3], mVerts[4],
                         mVerts[5], mVerts[6], mVerts[7]);
  ...
  for(int i = 0; i < List_Nbr(gModel->getGEOInternals()->PhysicalGroups); i++) {
    ...
    if(p->Typ == MSH_PHYSICAL_LINE) {
      ...
      if(List_Nbr(faceEntities) > 0) {
        ...
        PhysicalGroup *pnew =
          CreatePhysicalGroup(num, MSH_PHYSICAL_SURFACE, faceEntities);
...
//================================================================================
/*!
 * \brief modify the connectivity of face elements connected to the joint
 * elements to take into account the duplicates nodes. Nodes are picked in the
 * _nodeByHeavNode and _nodesByJunctionNode maps. In case of
 * _nodesByJunctionNode, the _heavFuncByJunctionNode map is used to determinate
...
//================================================================================
/*!
 * \brief modify node positions of the joint elements with distance eps. This
 * function is optional, it give a thickness to the joint elements that helps to
 * view the joint elements appear on the mesh. Nodes in the _nodeByHeavNode and
 * _nodesByJunctionNode maps are moved.
 */
//================================================================================
